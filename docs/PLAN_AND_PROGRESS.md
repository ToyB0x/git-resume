...
- [x] メタデータ検証・リンク切れチェック・構造検証
  ...
  - 検証システムの利用ガイド、自動化ガイド、プロンプト実践ガイドを作成
- [x] AIツール統合のためのルール整備
...
AIプロンプトベースの検証システムの整備が完了したため、以下のタスクを優先順位順に実施します：

1. **高優先度**: ドキュメント構造の重複解消（影響範囲大・実装容易性中）
   - 構造説明の一元化
   - 重要ディレクトリの明示的記載
   - `docs/rules/structure.md`、`docs/README.md`、`.clinerules`でのディレクトリ構造の説明の重複を解消

2. **中優先度**: 作成した検証システムを活用したドキュメント全体の検証実施（影響範囲大・実装容易性高）
   ```
