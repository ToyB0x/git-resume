...
注: 1次調査結果はキャッシュされないため、データモデルには含まれていません。1次調査は常に最新のGitHubプロフィール情報を取得します。
...
特に、GitHub User名入力後の3つの分岐（2次調査結果あり/2次調査実行中/2次調査未実行）を適切に処理するための状態管理設計により、ユーザーは常に適切な情報を得ることができます。1次調査結果をキャッシュせず常に最新データを取得することで、ユーザーは常に最新のGitHubプロフィール情報に基づいた調査計画を見ることができます。
