...
- 処理が軽量なため、結果はキャッシュせず常に最新データを取得

**2次調査（詳細分析段階）**：
- GitHubの複数のREST APIやGit Clone、AIによる解析などを行う重い処理
- CloudRun Jobsで時間をかけて実行
- バックグラウンドで処理が継続され、ユーザーはいつでも安全に離脱可能なアーキテクチャ
- 処理が重いため、結果は30日間キャッシュして再利用
- 詳細な分析ステップ：
  1. ユーザがGitHubでCommitしたリポジトリの検索（無料プラン過去1年分、有料プラン過去3年分）
  2. 見つかったリポジトリのClone
  3. 見つかったリポジトリについて、ユーザのCommitだけを1ファイルに書き出し（pack）
  4. Packについて、1リポジトリごとにAIで要約（リポジトリ活動のサマリ作成）
  5. 最後に、複数のサマリをAIが横断分析して、1枚のマークダウン形式のResume作成
...
2. **2次調査アーキテクチャ**
   ...
   - 重い処理のため、結果を30日間キャッシュして再利用する設計
   - 詳細分析パイプライン：
     - リポジトリ検索コンポーネント：ユーザのCommit履歴を検索
     - リポジトリCloneコンポーネント：検索結果に基づきリポジトリをClone
     - Commit抽出コンポーネント：ユーザのCommitのみを抽出しpack
     - AI要約コンポーネント：リポジトリごとの活動サマリを生成
     - AI横断分析コンポーネント：複数サマリからマークダウン形式のResumeを生成

3. **状態管理アーキテクチャ**
   - 調査状態（未実行/実行中/完了）の管理と適切な画面遷移の設計
   - 2次調査の進捗状況の追跡と表示のための構造（簡易表示のみ、詳細な中間結果保存は将来実装）
   - ユーザーの離脱と再訪問を考慮したセッション管理

4. **データ永続化アーキテクチャ**
   - 2次調査の結果と状態を効率的に保存するための構造
   - 30日間のデータ保持ポリシーを実現するための設計
   - キャッシュ戦略とデータベース設計
   - 中間結果の詳細保存は将来実装の目標とし、現段階では進捗ステップの簡易表示のみ
...
ResearchTask (2次調査タスク)
  ...
  - current_step: Enum (REPO_SEARCH, CLONE, PACK, SUMMARY, RESUME)
  - created_at: Timestamp
...
2. 中間結果の詳細保存は将来実装の目標とし、現段階では進捗ステップ（current_step）の簡易表示のみを保存します。
...
```
[調査開始] → [リポジトリ検索] → [リポジトリClone] → [Commit抽出(Pack)] → [リポジトリサマリ生成] → [Resume生成] → [完了]
```
...
特に、2次調査の詳細なステップ（リポジトリ検索、Clone、Commit抽出、サマリ生成、Resume生成）を明確に定義し、プラン別の処理範囲（無料: 1年分、有料: 3年分）を考慮した設計となっています。中間結果の詳細保存は将来実装の目標とし、現段階では進捗ステップの簡易表示のみを提供します。
