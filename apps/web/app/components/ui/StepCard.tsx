...
export function StepCard({ 
  ...
  const getStepIcon = (stepId: number, stepName: string) => {
    ...
    // デフォルトは数字を表示
    return <span className="text-xs font-medium text-blue-400">{stepId}</span>;
  };
  
  if (status === "completed") {
    statusClasses = "border-green-800 bg-opacity-20 bg-green-900";
    statusIcon = (
      <svg className="w-4 h-4 text-green-400 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    statusIcon = (
      <svg className="w-4 h-4 text-blue-400 mr-2 animate-pulse flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    statusIcon = (
      <div className="mr-2 opacity-70 flex-shrink-0">
